drop function if exists public.submit_quote_request(text, text, text, integer, text[], uuid, uuid, text[], uuid, uuid);
...
create function public.submit_quote_request(
  ...
  p_delivery_mode public.delivery_mode default 'warehouse',
...
begin
  ...
  if p_supersedes_quote_id is not null then
    insert into public.quote_requests
      (store_id, product_url, product_name, notes, target_monthly_volume, image_urls, status,
       supersedes_quote_id, target_countries, preview_id, delivery_mode, delivery_address)
    select v_store.id, q.product_url, q.product_name, q.notes, q.target_monthly_volume, q.image_urls,
           'submitted', q.id, q.target_countries, q.preview_id, q.delivery_mode, q.delivery_address
    ...
    insert into public.quote_requests
      (store_id, product_url, product_name, notes, target_monthly_volume, image_urls, status,
       target_countries, preview_id, delivery_mode, delivery_address)
    values
      (v_store.id, p_product_url, p_product_name, p_notes, p_target_monthly_volume,
       coalesce(p_image_urls, '{}'), 'submitted', v_countries, p_preview_id,
       coalesce(p_delivery_mode, 'warehouse'), nullif(trim(coalesce(p_delivery_address, '')), ''))
    returning * into v_row;
...
revoke all on function public.submit_quote_request(text, text, text, integer, text[], uuid, uuid, text[], uuid, uuid, public.delivery_mode, text) from public, anon;
grant execute on function public.submit_quote_request(text, text, text, integer, text[], uuid, uuid, text[], uuid, uuid, public.delivery_mode, text) to authenticated, service_role;
