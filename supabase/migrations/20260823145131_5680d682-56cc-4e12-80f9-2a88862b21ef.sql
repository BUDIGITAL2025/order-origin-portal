-- 48h sourcing SLA on quote requests
alter table public.quote_requests
  add column if not exists quote_due_at timestamptz,
  add column if not exists quote_breach_notified_at timestamptz;

update public.quote_requests
   set quote_due_at = created_at + interval '48 hours'
 where quote_due_at is null;

alter table public.quote_requests
  alter column quote_due_at set not null,
  alter column quote_due_at set default (now() + interval '48 hours');

-- Store connection is never required to submit a quote: the only hard gate is
-- an active subscription with quota. A storeless subscribed account gets a
-- draft workspace created on the fly (subscription state reconstructed from
-- the entity's subscription receipts, if any).
...
begin
  ...
  if p_store_id is not null then
    ...
    -- Prefer a workspace that can actually submit (subscribed or fee-waived),
    -- then fall back to the account's oldest workspace.
    select s.* into v_store
      from public.stores s
      join public.entities e on e.id = s.entity_id
    ...
    if not found then
      -- Draft-first flow: create the workspace on the fly. If the entity has a
      -- subscription receipt but its workspace row went missing (edge case from
      -- earlier flows), reconstruct the workspace as subscribed.
  ...
  if p_supersedes_quote_id is null then
    -- Paywall: quoting requires an active subscription on this workspace.
...
-- Admins need to see SLA-breach notifications; clients keep their scoped access.
