...
export function createStripeClient(env: StripeEnv): Stripe {
  ...
  return new Stripe(connectionApiKey, {
    // Pin the wire API version explicitly so request/response shapes don't
    // silently change if a future SDK bump lands a new default.
...
/**
 * Verify a Stripe webhook signature (HMAC-SHA256) without depending on the
 * SDK's network calls — the gateway proxy is only needed for API calls, not
